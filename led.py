...
def play_audio_with_led(filename, speed=0.08, r=255, g=0, b=0, dynamic_acceleration=False, color_fade_to_yellow=False, dynamic_deceleration=False):
    ...
    while process.poll() is None:
        ...
        # 1. SPM gyorsulási logika (0.08-ról 0.035-re)
        if dynamic_acceleration:
            progress = min(elapsed / estimated_duration, 1.0)
            current_speed = 0.08 - (progress * (0.08 - 0.035))
            
        # 2. Leállítási lassulási logika: 0.08-as alapértékről fokozatosan lassul 0.25-re
...
def process_and_respond():
    ...
    try:
        ...
        txt = r.recognize_google(audio_data, language="hu-HU").lower().strip()
        ...
        if txt:
            if any(x in txt for x in ["leállítás", "leallitas", "kikapcsolás", "kikapcsolas"]):
                ...
                # Hozzáadva a dynamic_deceleration=True a fokozatos elhaláshoz
            ...
            elif any(x in txt for x in ["bemutatás", "bemutatas", "bemutatása", "bemutatasa", "ismertetés", "ismertetes", "ismertetése", "ismertetese", "karr"]):
